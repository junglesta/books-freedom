...
3. **Test** — `pnpm test`
4. **Build** — `pnpm build`
5. **Version bump** — update `version` in `package.json` (usually patch bump unless user asks otherwise)
6. **Changelog** — update `CHANGELOG.md` with a short entry for the new version
