<script lang="ts">
  ...
  import { scanIsbn } from '../lib/api';
  import { addBookToCollection, showToast, navigate } from '../lib/stores.svelte.ts';
  ...
  {#if preview}
    <div class="scan-preview" class:added={justAdded}>
      ...
      <div class="preview-actions">
        {#if justAdded}
          <button class="btn btn-primary" onclick={viewInLibrary}>View in Library</button>
          <button class="btn btn-secondary" onclick={scanAnother}>Scan Another</button>
        {:else if preview.alreadyExists}
          <button class="btn btn-secondary" onclick={viewInLibrary}>View in Library</button>
          <button class="btn btn-ghost" onclick={scanAnother}>Dismiss</button>
        {:else}
